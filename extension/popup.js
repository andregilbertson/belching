...
document.addEventListener('DOMContentLoaded', () => {
  ...
  getPromptBtn.addEventListener('click', async () => {
    try {
      ...
      if (!tab.url || tab.url.startsWith('chrome://') || tab.url.startsWith('chrome-extension://') || tab.url.startsWith('edge://')) {
        updateStatus('Error: Content scripts cannot run on this page. Please navigate to a regular web page.');
      ...
      try {
        ...
        console.log('Direct injection failed, trying content script approach:', directError);
  ...
  async function injectPromptText(tabId, selector) {
    ...
    if (!chrome.scripting || !chrome.scripting.executeScript) {
      throw new Error('chrome.scripting API is not available. Please reload the extension after the manifest update.');
    }
  ...
  async function injectAndRetryPrompt(tabId, selector) {
    try {
      ...
      try {
        // Check if scripting API is available
        if (!chrome.scripting || !chrome.scripting.executeScript) {
          throw new Error('chrome.scripting API is not available. Please reload the extension.');
        }
        ...
        if (messageResponse && !messageResponse.error && messageResponse.success) {
        ...
        console.log('File injection failed, trying direct function injection:', fileError);
        await injectPromptText(tabId, selector);
      }
    } catch (injectError) {
  ...
  async function showErrorDetails(error) {
    ...
    const isScriptingError = errorMessage.includes('scripting') || errorMessage.includes('executeScript');
    ...
    if (response && response.success) {
      ...
      textOutput.innerHTML = `
        <div class="text-info">
          ...
          ${response.elementInfo ? `<strong>Source:</strong> ${escapeHtml(response.elementInfo)}<br>` : ''}
      ...
      updateStatus(`Retrieved ${charCount.toLocaleString()} characters from page`);
    } else {
